...
describe Forms::Syncs::Create, type: :form do
  ...
  context 'when customer' do
    let(:attributes) { Util::InputHelpers::Customer.new.sync_request_body(account: account) }
    ...
    it { expect_error_with_missing_attribute(:account_external_id) }
  ...
  context 'when payment' do
    let(:attributes) { Util::InputHelpers::Payment.new.sync_request_body(account: account) }
