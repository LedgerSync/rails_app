# frozen_string_literal: true

require 'rails_helper'
require 'formify/spec_helpers'

describe Forms::LedgerResources::Assign, type: :form do
  include Formify::SpecHelpers

  let(:ledger_resource) { FactoryBot.create(:ledger_resource, resource_ledger_id: nil) }
  let(:resource_ledger_id) { 'RESOURCE_LEDGER_ID_VALUE' }
  let(:resource) { ledger_resource.resource }

  let(:attributes) do
    {
      ledger_resource: ledger_resource,
      resource_ledger_id: resource_ledger_id
    }
  end

  it { expect(ledger_resource.resource_ledger_id).to be_nil }
  it { expect_valid }
  it { expect(result).to be_success }
  it { expect(value).to be_a(LedgerResource) }
  it { expect(value.resource_ledger_id).to eq(resource_ledger_id) }

  describe '#resource_ledger_id' do
    it { expect_error_with_missing_attribute(:resource_ledger_id) }

    context 'when already assigned' do
      before { ledger_resource.update!(resource_ledger_id: :asdf) }

      it { expect_not_valid(attribute: :resource_ledger_id, message: t('ledger_resources.assign.already_assigned')) }
    end
  end

  context 'with sync' do
    let(:sync) { FactoryBot.create(:sync_resource, resource: resource).sync }

    before { Forms::Syncs::UpdateStatus.new(sync: sync).save }

    it do
      expect(sync.reload).to be_blocked
      value
      expect(sync.reload).to be_queued
    end
  end
end
